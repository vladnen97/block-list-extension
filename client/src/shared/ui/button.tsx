...
export const Button = ({ className, variant = 'primary', ...props }: Props) => {
  return (
    <button
      ...
      className={clsx(
        className,
        'px-4 h-10 rounded cursor-pointer flex ga-2 items-center justify-center disabled:cursor-not-allowed disabled:opacity-50 transition duration-200',
        {
          primary:
          ...
          outline:
            'border border-slate-300 [&:hover:not(:disabled)]:border-slate-500',
        }[variant]
      )}
      {...props}
    />
